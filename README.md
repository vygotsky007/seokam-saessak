...
- 🛠 관리자 콘솔
  ...
  - 🧭 기록 참고(선정 보조, **교사 전용·비공개**): 학생 기록(긍정/부정) 점수를 바탕으로
    ①긍정 우선 정렬 ②부정 후순위 ③긍정 학생 자동 선정(정원 내). 모두 on/off,
    자동 선정은 강제가 아니며 "저장" 전까지 DB 미반영 + "자동선정 해제"로 되돌리기.
    점수·긍부정은 학생·학부모 화면/공개 API에 절대 노출되지 않음(서버 403 게이트).
    **DB 변경 없음** — 기존 `student_notes` 테이블만 읽음(마이그레이션 불필요).
  - 학생 → 다른 프로그램으로 1클릭 복사
...
세 가지 유형이 있다. 자동 점수화나 자동 선발은 하지 **않는다.** 우대는 관리자가 수동 판단할 때 참고하는 메타데이터일 뿐이다.

| 유형 | program_type | 추가 필드 | 공개 폼 동작 |
|---|---|---|---|
| 일반형 | `general` | — | 학년/정원만 검증 |
| 다문화 우대형 | `multicultural` | `multicultural_min` (정원 중 최소 보장 인원) | 이 프로그램을 체크한 학생에게만 "다문화가정 여부 (선택)" 질문 노출 |
| 형제 우대형 | `sibling` | — | 형제 동시 신청 시 자동 부여되는 `sibling_group_id`로 추적 (별도 입력 없음) |

운영 가이드:

- 공개 폼에는 "다문화/형제가 유리하다"는 안내·점수를 절대 표시하지 않는다. 프로그램 카드에 유형 태그만 담백하게 보인다.
- 관리자 종합 대시보드에서 다문화 우대형 프로그램마다 **"다문화 ○명 / 최소 △명"** 진행바를 본다. 최소 미달이면 빨간색, 충족이면 초록색.
- 형제 동시 신청은 공개 폼의 `+ 형제·자매 추가` 버튼으로 학생 블록을 늘려서 접수하면 같은 `sibling_group_id`로 묶인다. 관리자 화면 신청자 명단에서 같은 색상 배지로 묶음을 식별한다.
- 엑셀 내보내기에는 `프로그램유형`, `다문화여부`, `형제묶음ID` 컬럼이 포함된다.
